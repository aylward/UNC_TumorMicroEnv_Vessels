...
class usangio_vessels:
    ...
    def read_matlab_file(self, filename, nx=460, ny=308, nz=510, bits=32):
        print("Reading MATLAB file", flush=True)
        ...
        if bits == 32:
            ...
            print("ERROR: Only 16, 32, and 64 bit floats supported", flush=True)
            self.data_im = []
            return
    ...
    def import_matlab_data(self, data):
        print("Importing MATLAB data", flush=True)
    ...
    def generate_training_mask_image(self):
        ...
        print("   - Computing training mask (This takes several minutes)", flush=True)
    ...
    def extract_vessels(self, level_of_detail=0):
        ...
        itk.imwrite(imVessMask, self.output_base_name + str(level_of_detail) + "-VesselSeedsInitialMask.mha")
        imVessMask = itk.imread(self.output_base_name + str(level_of_detail) + "-VesselSeedsInitialMask.mha", itk.F)
        ...
        itk.imwrite(imSeeds, self.output_base_name + str(level_of_detail) + "-VesselSeeds.mha")
        ...
        itk.imwrite(
            imSeedsRadius, self.output_base_name + str(level_of_detail) + "-VesselSeedsRadius.mha"
        )

        if level_of_detail==0:
            imMath.SetInput(self.data_im)
        ...
        itk.imwrite(imInput, self.output_base_name + str(level_of_detail) + "-VesselInput.mha")
        ...
        itk.imwrite(
            self.data_vessels_im, self.output_base_name + str(level_of_detail) + "-Vessels.mha"
        )
        ...
        SOWriter.SetFileName(self.output_base_name + str(level_of_detail) + "-Vessels.tre")
        SOWriter.Update()

        ConvSurface = tube.WriteTubesAsPolyData.New()
        ConvSurface.SetInput(self.data_vessels_group)
        ConvSurface.SetFileName(self.output_base_name + str(level_of_detail) + "-Vessels.vtp")
        ConvSurface.Update()

    def run(self, level_of_detail=1):
        self.normalize_image()
        self.identify_training_seeds()
        self.generate_training_mask_image()
        self.enhance_vessels()
        self.extract_vessels(level_of_detail)
